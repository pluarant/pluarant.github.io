...
strings.pushnotification = "You can send push notification to users on receiving webhook calls. For example, if user is offline and a message is received for that user, mesibo will invoke your webhook, and in turn, you can invoke appropriate push notification to user depending on which device user is on.";
...
routerApp.controller('headerController', ['$scope', '$window', '$state', function($scope, $window, $state) {
...
routerApp.controller('appcontroller', ['$scope', '$window', '$state', '$stateParams', function($scope, $window, $state, $stateParams) {
   ...
   // alert('app: ' + name);
    ...
    $scope.save_general = function() {
	 ...
	 mesibo_appset($scope.user.app.token, $scope.user.app.name, $scope.user.app.maxusers, $scope.user.app.maxgroups, $scope.user.app.maxmsgs, '', -1);
	 $scope.savetoast();
    };
    
    $scope.save_webhooks = function() {
	 mesibo_appset($scope.user.app.token, '', -1, -1, -1, $scope.user.app.url, $scope.user.app.notify);
	 $scope.savetoast();
    };
...
routerApp.controller('accountController', ['$scope', '$window', '$state', function($scope, $window, $state) {
...
routerApp.controller("mainCtrl", ['$scope', '$rootScope', '$window', '$state', '$location', '$http', 'toastr', function($scope, $rootScope, $window, $state, $location, $http, toastr){
		...
		if(!absurl.startsWith('http://127.0.0.1')) {
			mesibo_setapi("https://mesibo.com/api/api.php");
			/* Log('setting api: https://mesibo.com/develapi/api.php'); */
		}
			mesibo_setapi("https://mesibo.com/api/api.php");
